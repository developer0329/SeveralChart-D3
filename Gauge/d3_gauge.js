  ...
  svg = el.append('svg')
	...
	for(var i = 0 ; i <= 10; i++)
	{
		if(i == 0 || i == 5 || i == 10)
		{
			chart.append('text')
					.attr('transform', function() {
						if(i == 0)
						{
							return 'rotate(0) translate(-'+radius+',0)';
						}
						else if(i == 5)
						{
							return 'rotate(0) translate(0,-'+radius+')';
						}	
						else if(i == 10)
						{
							return 'rotate(0) translate('+radius+',0)';
		...
		else	
		{
			chart.append('circle')
				.attr('class', "plot")
				.attr('cx', -radius * Math.cos(Math.PI / 10*i))
				.attr('cy', -radius * Math.sin(Math.PI / 10*i))
				.attr('r', 3);
		}
	}
	...
	var Needle = (function() {
		...
		**/
		var recalcPointerPos = function(perc) {
			...
			leftX = centerX - this.radius * Math.cos(thetaRad - Math.PI / 12);
			leftY = centerY - this.radius * Math.sin(thetaRad - Math.PI / 12);
			rightX = centerX - this.radius * Math.cos(thetaRad + Math.PI / 12);
			rightY = centerY - this.radius * Math.sin(thetaRad + Math.PI / 12);
		...
		Needle.prototype.moveTo = function(perc) {
			...
			var value = d3.format(".1f");
			
			d3.select("#needle-center-label")
				.transition()
				.duration(200)
				.text(value(perc*10));
